...
2. Настроенный TelegramMCP с bot token
...
| Реальная отправка | ⚠️ | Требует канал + TelegramMCP |
...
**Необходимо:**
1. Добавить Telegram канал:
   ```sql
   INSERT INTO telegram_channels (user_id, channel_name, chat_id, is_active, is_default)
   VALUES (5, 'Test Channel', -1001234567890, 1, 1);
   ```
2. Настроить TelegramMCP (bot token)
3. Запрос с `test_mode: false`
...
3. `test_content_create_flow.py` (+223 строки, новый)
