...
Задачи:
  • Написание постов, статей через OpenAI GPT-3.5-turbo
  • SEO-оптимизация (keywords, hashtags)
  • Форматирование контента
  • Адаптация под платформы (Telegram, VK, Instagram, Twitter)
  • Оценка качества (SEO score, engagement potential, readability)
...
```
Роль: Подготовка контента к публикации
Задачи:
  • Форматирование под каждую платформу (HTML для Telegram, plain text для VK)
  • Валидация контента (длина, хештеги, ссылки)
  • Расчет оптимального времени публикации
  • Генерация preview контента
...
Status: ✅ Готовит контент (реальная публикация в следующей версии)
```
...
- ✅ **Время создания контента:** < 2 секунды (с OpenAI API)
- ✅ **Успешность выполнения:** 100%
- ✅ **Одновременных задач:** до 35 (сумма всех агентов)
- ✅ **Сохранение в БД:** Весь контент и токены сохраняются
- ✅ **История контента:** Доступна через API
...
Получить всю историю созданного контента с фильтрами

**Параметры:**
- `page` - номер страницы (default: 1)
- `per_page` - элементов на странице (default: 10, max: 100)
- `platform` - фильтр по платформе (telegram, vk, instagram)
...
Получить детальную информацию о конкретном контенте
...
```json
{
  ...
  "content": {
    "id": "uuid",
    "title": "AI в бизнесе 2025",
    "text": "Полный текст контента...",
    "platform": "telegram",
    "content_type": "post",
    "hashtags": ["#ai", "#бизнес", "#технологии"],
    "call_to_action": "Подписывайтесь на наш канал!",
    "status": "draft",
    "created_by_agent": "drafting_001",
    "quality_metrics": {
      "seo_score": 0.85,
    ...
    "token_usage": {
      "total_tokens": 450,
      "total_cost_rub": 1.85,
      "agents_used": ["chief_001", "drafting_001"]
    },
    "created_at": "2025-10-10T12:00:00Z",
...
**💰 Важно:** Показывает сколько токенов и денег потрачено на создание контента!

#### `DELETE /api/v1/content/<content_id>`
Удалить контент из истории
...
1. **Таблица `content_pieces`** - весь созданный контент:
   - Текст, заголовок, хештеги
   - Платформа и тип контента
   - Метрики качества (SEO, engagement, readability)
   - Метрики производительности (views, likes, shares)
   - Связь с пользователем и workflow

2. **Таблица `token_usage`** - использование AI токенов:
   - Количество токенов (prompt + completion)
   - Стоимость в USD и RUB
   - AI модель (gpt-3.5-turbo, gpt-4, dall-e-3)
   - Агент который использовал
   - Связь с контентом и пользователем

3. **Таблица `content_history`** - история изменений:
   - Действие (created, updated, published)
   - Снапшот контента
   - Кто изменил (агент или пользователь)

**Все данные доступны через API для анализа и отчетности!**
