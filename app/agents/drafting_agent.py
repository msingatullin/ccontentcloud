    ...
    def _load_ai_prompts(self) -> Dict[str, ContentPrompt]:
        ...
        return {
            "telegram_post": ContentPrompt(
                platform="telegram",
                content_type="post",
                prompt_template="""Создай короткий живой пост для Telegram на тему: {topic}

Пиши так, будто ты умный друг, который делится рабочим лайфхаком. Естественный язык, минимум воды. Никаких корпоративных фраз, шаблонов, «узнайте подробнее», «это реальность», «инструменты помогают». Никакой теории ради теории — только польза.

Стиль: плотный, понятный, человеческий. В каждом предложении — смысл. Легкая эмоциональность допустима, но без продажной истерики.

Структура:

1) Зацепка — что-то неожиданное, честное, наблюдение или инсайт (1 предложение)

2) Суть — конкретная мысль, польза или применимый вывод (2 предложения)

3) Мягкое действие — подписаться, сохранить, подумать (1 короткая строка)
...
— Никаких описаний "тема поста:", "целевой аудитории" и т.п.

— Не повторять формулировки из задания.

Теперь создай пост на тему: {topic}""",
                max_tokens=250,
                temperature=0.8,
                system_message="Ты умный друг, который делится рабочими лайфхаками. Пишешь плотно, понятно, человечески. Никаких корпоративных фраз и шаблонов. Только конкретная польза в каждом предложении.",
                examples=[
