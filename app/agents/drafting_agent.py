    ...
    def _load_ai_prompts(self) -> Dict[str, ContentPrompt]:
        ...
        return {
            "telegram_post": ContentPrompt(
                platform="telegram",
                content_type="post",
                prompt_template="""Создай УВЛЕКАТЕЛЬНЫЙ и ПОЛЕЗНЫЙ пост для Telegram канала.
...
КРИТИЧЕСКИ ВАЖНО:
- Пиши ЖИВЫМ языком, как профессиональный копирайтер, а НЕ как AI-бот
- ЗАПРЕЩЕНО использовать шаблонные AI-фразы:
  ❌ "Целевая аудитория включает..."
  ❌ "Для ... важно понимать..."
  ❌ "В современном мире..."
  ❌ "Как показывает практика..."
- НЕ копируй техническое описание темы - ИНТЕРПРЕТИРУЙ его креативно
- Используй КОНКРЕТИКУ: цифры, факты, реальные примеры, кейсы
- Добавь 1-2 уместных эмодзи (не переборщи!)
- Пиши так, чтобы читатель остановился и прочитал до конца

Структура поста (300-500 символов):
1. ЗАЦЕПКА (1 предложение): Сильный вопрос, парадокс, провокация или неожиданный факт
2. ПОЛЬЗА (2-3 предложения): Конкретное решение, инсайт или ценная информация
3. ПРИЗЫВ (1 предложение): Простое и четкое действие для читателя

ОТЛИЧНЫЕ ПРИМЕРЫ:
✅ "🔥 80% стартапов умирают в первый год. Знаете, что спасло остальные 20%? Всего одна цифра — unit-экономика. Рассказываем, как считать."

✅ "💡 Вы тратите 3 часа в день на email? А знали, что есть метод Inbox Zero — обрабатывайте почту за 15 минут. Попробуйте сегодня."

✅ "📊 Ваш конкурент поднял цены на 30% и продажи выросли. Парадокс? Нет, психология ценообразования. Разбираем 3 приема."

ПЛОХИЕ ПРИМЕРЫ (НЕ ДЕЛАЙ ТАК):
❌ "Целевая аудитория включает молодых профессионалов. Для них важно понимать основы инвестиций..."
❌ "В современном мире автоматизация контента становится все более актуальной темой..."
❌ "Разбираем тему: искусственный интеллект. Важно знать про AI..."

Теперь создай СИЛЬНЫЙ пост на тему: {topic}
Помни: читатель должен остановить скролл и прочитать!""",
                max_tokens=300,
                temperature=0.9,
                system_message="Ты профессиональный копирайтер с опытом viral-маркетинга. Твои посты останавливают скролл, цепляют и дают конкретную пользу. Пишешь живо, понятно, БЕЗ воды и AI-штампов. Каждое слово работает.",
                examples=[
                    "🚀 Запустили продукт за $0 — выручка $50K в первый месяц. Как?",
                    "💰 Клиент платит $10K за консультацию на 2 часа. Секрет ценообразования",
                    "⚡ 1 промпт вместо 10 часов работы. ChatGPT для ленивых (но умных)"
                ]
            ),
...
3. Хештеги""",
    ...
    async def _generate_hook(self, brief_data: Dict[str, Any],
        ...
        # СИЛЬНЫЕ зацепки вместо шаблонных фраз
        import random

        # Типы зацепок по категориям
        hook_templates = {
            "question": [
                f"Почему {topic} работает не у всех?",
                f"Что вы НЕ знали про {topic}?",
                f"Как выбрать {topic} без ошибок?",
                f"{topic.capitalize()}: а вы уверены, что делаете правильно?"
            ],
            "problem": [
                f"3 главные ошибки с {topic}",
                f"{topic.capitalize()}: типичные проблемы и решения",
                f"Почему {topic} не дает результата?",
                f"Что мешает получить от {topic} максимум?"
            ],
            "benefit": [
                f"Как {topic} поможет вам сэкономить время",
                f"{topic.capitalize()}: быстрый способ улучшить результат",
                f"Что даст вам {topic} уже сегодня",
                f"{topic.capitalize()} — простой способ решить проблему"
            ],
            "insight": [
                f"Неочевидный факт про {topic}",
                f"{topic.capitalize()}: что знают профи, а новички — нет",
                f"Секрет {topic}, о котором молчат",
                f"Чему нас научил опыт с {topic}"
            ],
            "urgency": [
                f"{topic.capitalize()}: почему важно действовать сейчас",
                f"Как НЕ упустить возможности с {topic}",
                f"{topic.capitalize()}: пока не поздно",
                f"Время действовать: {topic} в 2025"
            ]
        }

        # Выбираем категорию в зависимости от тона
        tone_to_category = {
            "professional": ["insight", "benefit", "problem"],
            "casual": ["question", "problem", "benefit"],
            "friendly": ["question", "benefit", "insight"],
            "authoritative": ["insight", "problem", "urgency"]
        }

        available_categories = tone_to_category.get(tone, ["question", "benefit", "problem"])

        # Добавляем вариативность для разных вариантов поста
        if variant_num > 1:
            # Для второго варианта используем другую категорию
            available_categories = available_categories[1:] + available_categories[:1]

        category = random.choice(available_categories)
        hook = random.choice(hook_templates[category])

        # Добавляем эмодзи в зависимости от платформы и категории
