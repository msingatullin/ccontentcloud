    ...
    def _load_ai_prompts(self) -> Dict[str, ContentPrompt]:
        ...
        return {
            "telegram_post": ContentPrompt(
                platform="telegram",
                content_type="post",
                prompt_template="""Напиши пост для Telegram на тему: {topic}

Целевая аудитория: {target_audience}
Тон: {tone} (но естественный, как будто пишешь другу или коллеге)
Ключевые слова: {keywords}

КРИТИЧЕСКИ ВАЖНО - ПИШИ КАК ЖИВОЙ ЧЕЛОВЕК:
- НЕ используй маркетинговые клише: "узнайте секрет", "революционное решение", "ваш ключ к успеху", "откройте для себя", "не упустите возможность"
- НЕ используй шаблонные фразы типа "будущее уже здесь", "измените свою жизнь", "станьте лучшей версией себя"
- НЕ используй шаблонные зацепки: "💡 Открываем секреты", "🚀 Новые технологии меняют", "💡 Знаете ли вы, что...", "🎯 Хотите узнать секрет"
- НЕ используй эмодзи в начале как шаблонную зацепку - только если это естественно для содержания
- Пиши просто и понятно, используй конкретные факты из описания
- Допускай разговорные обороты и неформальности где уместно
- Используй личный опыт и примеры где возможно ("я заметил", "в моем опыте", "сталкивался с")
- Избегай излишней формальности и "продающего" тона
- Будь конкретным, не общими фразами
- Пиши как эксперт, который делится знаниями, а не как маркетолог
- НИКОГДА не добавляй метаданные типа "Наши цели:", "Бизнес-цели:" или любые технические данные в текст поста

Требования к структуре:
1. Зацепка (1-2 предложения) - на основе реальной темы из описания, НЕ шаблонная
2. Основной контент (10-15 предложений) - РАСКРОЙ тему из описания, используй конкретные факты и информацию
3. Призыв к действию (1 предложение) - естественный, не навязчивый

Длина: до 1500 символов, пост должен быть ЗАВЕРШЕННЫМ и информативным.
Используй эмодзи умеренно (2-3 эмодзи).
НЕ добавляй блоки "Похожие события", "Similar events", исторические данные или события за последние годы.
НЕ обрывай пост на полуслове - обязательно заверши мысль.""",
                max_tokens=2000,  # Увеличено для длинного поста (1500 символов требует ~1125-1500 токенов, берем с запасом)
                temperature=0.85,  # Повышено для большей естественности и вариативности
                system_message="""Ты опытный контент-мейкер, который пишет посты для социальных сетей. Твоя главная задача - писать так, чтобы пост выглядел как написанный живым человеком, а не ИИ.

КРИТИЧЕСКИ ВАЖНО:
- НЕ используй шаблонные маркетинговые фразы: "узнайте секрет", "революционное решение", "ваш ключ к успеху", "откройте для себя", "не упустите возможность"
- НЕ используй клише: "будущее уже здесь", "измените свою жизнь", "станьте лучшей версией себя"
- НЕ используй шаблонные зацепки с эмодзи: "💡 Открываем секреты", "🚀 Новые технологии", "💡 Знаете ли вы", "🎯 Хотите узнать секрет"
- НЕ начинай пост с эмодзи-зацепок - используй эмодзи только естественно в контексте
- Пиши просто и понятно, как будто объясняешь другу или коллеге
- Используй конкретные факты и примеры из описания темы
- Допускай небольшие неформальности и разговорные обороты
- Избегай излишней формальности и "продающего" тона
- Пиши от первого лица где уместно ("я заметил", "в моем опыте", "сталкивался с")
- Не бойся использовать простые слова вместо сложных терминов
- Добавляй личные наблюдения и реальный опыт
- Будь конкретным, избегай общих фраз
- НИКОГДА не добавляй метаданные типа "Наши цели:", "Бизнес-цели:", технические данные или ID в текст поста

Цель: пост должен читаться как написанный реальным человеком, который разбирается в теме и делится знаниями, а не как рекламный текст или ИИ-генерация.
НЕ добавляй блоки с похожими событиями, историческими данными или событиями за последние годы.""",
                examples=[
                    "Вчера столкнулся с проблемой X. Оказалось, решение простое - нужно Y. Вот как я это сделал...",
                    "Многие спрашивают про Z. На самом деле, это не так сложно, как кажется. Расскажу на своем примере...",
                    "Работаю с этим уже 3 года. Заметил, что большинство людей делают одну и ту же ошибку..."
                ]
            ),
            "vk_post": ContentPrompt(
                platform="vk",
                content_type="post",
                prompt_template="""Напиши пост для VK на тему: {topic}

Целевая аудитория: {target_audience}
Тон: {tone} (естественный, разговорный)
Ключевые слова: {keywords}

ВАЖНО - ПИШИ КАК ЖИВОЙ ЧЕЛОВЕК:
- НЕ используй маркетинговые клише и шаблонные фразы
- Пиши в разговорном стиле, как будто общаешься с друзьями
- Используй конкретные факты из описания
- Допускай неформальности и разговорные обороты
...
Структура:
1. Зацепка с вопросом (естественная, не шаблонная)
2. Основная информация (конкретная, из описания)
3. Призыв к обсуждению (не навязчивый)""",
                max_tokens=150,
                temperature=0.85,  # Повышено для естественности
                system_message="""Ты создаешь контент для VK - социальной сети с живым общением. Пиши в разговорном стиле, как живой человек общается с друзьями. 

КРИТИЧЕСКИ ВАЖНО:
- НЕ используй маркетинговые клише и шаблонные фразы
- Пиши просто и естественно, используй разговорные обороты
- Задавай вопросы для вовлечения, но естественные, не шаблонные
- Используй конкретные факты из описания темы
- Будь дружелюбным, но не навязчивым

Цель: пост должен выглядеть как написанный реальным человеком в личной переписке или обсуждении.
НЕ добавляй блоки с похожими событиями, историческими данными или событиями за последние годы.""",
                examples=[
                    "Вчера попробовал X и вот что получилось. А вы пробовали?",
                    "Столкнулся с такой ситуацией: Y. Кто-нибудь сталкивался? Как решали?",
                    "Уже месяц использую Z. Заметил интересную особенность... А что думаете?"
                ]
            ),
            "instagram_post": ContentPrompt(
                platform="instagram",
                content_type="post",
                prompt_template="""Напиши пост для Instagram на тему: {topic}

Целевая аудитория: {target_audience}
Тон: {tone} (естественный, живой)
Ключевые слова: {keywords}

ВАЖНО - ПИШИ КАК ЖИВОЙ ЧЕЛОВЕК:
- НЕ используй шаблонные фразы типа "вдохновение на каждый день", "новые возможности ждут"
- Пиши естественно, используй конкретные факты из описания
- Используй эмодзи, но не переборщи

Требования:
- Визуально привлекательный текст
- Эмодзи (2-4 штуки, умеренно)
- Хештеги для охвата
- Длина: до 150 символов
- НЕ добавляй блоки "Похожие события", "Similar events", исторические данные или события за последние годы

Структура:
1. Эмодзи + зацепка (естественная, не шаблонная)
2. Краткая информация (конкретная, из описания)
3. Хештеги""",
                max_tokens=100,
                temperature=0.9,
                system_message="""Ты создаешь контент для Instagram - визуальной платформы. Пиши естественно, как живой человек делится моментом или мыслью.

КРИТИЧЕСКИ ВАЖНО:
- НЕ используй шаблонные фразы: "вдохновение на каждый день", "новые возможности ждут", "момент для изменений"
- Пиши просто и естественно, используй конкретные факты из описания
- Используй эмодзи умеренно, создавай атмосферу
- Будь конкретным, избегай общих фраз

Цель: пост должен выглядеть как написанный реальным человеком, который делится реальным опытом или мыслью.
НЕ добавляй блоки с похожими событиями, историческими данными или событиями за последние годы.""",
                examples=[
                    "Вчера попробовал новый подход к X. Результат удивил! ✨",
                    "Работаю над Y уже месяц. Вот что заметил... 💡",
                    "Столкнулся с Z. Оказалось проще, чем думал. Кто пробовал? 🚀"
                ]
            ),
            "twitter_post": ContentPrompt(
                platform="twitter",
                content_type="post",
                prompt_template="""Напиши твит на тему: {topic}
...
ВАЖНО - ПИШИ КАК ЖИВОЙ ЧЕЛОВЕК:
- НЕ используй шаблонные фразы типа "идея дня", "горячая тема", "быстрый факт"
- Пиши остроумно и лаконично, используй конкретные факты из описания
...
Структура:
1. Зацепка (естественная, не шаблонная)
2. Основная мысль (конкретная, из описания)
3. Хештеги""",
                max_tokens=100,
                temperature=0.85,  # Повышено для естественности
                system_message="""Ты создаешь твиты - короткие, остроумные сообщения. Пиши как живой человек делится мыслью или наблюдением.

КРИТИЧЕСКИ ВАЖНО:
- НЕ используй шаблонные фразы: "идея дня", "горячая тема", "быстрый факт"
- Будь лаконичным и запоминающимся, но естественным
- Используй конкретные факты из описания темы
- Пиши остроумно, но не навязчиво
- Избегай формальности

Цель: твит должен выглядеть как написанный реальным человеком, который делится реальной мыслью или наблюдением.
НЕ добавляй блоки с похожими событиями, историческими данными или событиями за последние годы.""",
                examples=[
                    "Попробовал X. Оказалось, что Y работает лучше, чем я думал.",
                    "Заметил интересную закономерность: когда делаешь Z, получается A. Кто еще замечал?",
                    "Месяц работаю с B. Вот что выяснил на практике..."
                ]
            )
        }
    ...
    async def _generate_content_with_ai(self, brief_data: Dict[str, Any], 
        ...
        try:
            ...
            if description and len(description) > 50:
                ...
                final_prompt += f"\n\nДетальное описание темы (ОБЯЗАТЕЛЬНО используй эту информацию для создания контента):\n{description_to_use}"
                if len(description) > max_desc_length:
                    final_prompt += f"\n\n(описание обрезано, но используй ключевые моменты из начала)"
                
                # Добавляем важное напоминание об использовании контекста сферы
                final_prompt += f"\n\nВАЖНО: Описание содержит контекст твоей сферы деятельности. Используй терминологию, примеры и стиль, характерные для этой области. Но пиши естественно, как эксперт, который делится знаниями, а не как маркетолог или ИИ-ассистент."
